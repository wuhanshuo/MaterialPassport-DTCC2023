...
def findRecord(file,db,ID):
    data =  {'db':'0','ID': '404', 'projectName': 'Not Found'}   # not found page defination
...
def index():
    ...
    if not (get_id and db):
        data = {'db':'0','ID': '', 'projectName': 'Database Group', 'magic':'Congratulations, Sherlock! You have stumbled upon a mysterious hidden page.'}
    elif not (get_id.isdecimal() and get_id.isdecimal()):
        data = {'db':'0','ID': '404', 'projectName': 'Not Found'}
    else:
        try:
